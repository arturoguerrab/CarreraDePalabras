...
export const processRoundResults = async (io, roomId, room) => {
	try {
		...
		// Identificar palabras únicas para validar
		...
		io.to(roomId).emit("round_results", {
			...
			scores,
			isGameOver,
			round: isGameOver ? currentRound : currentRound - 1,
		...
		try {
			await Room.updateOne({ roomId }, { isCalculating: false });
		} catch (err) {
			logger.error("Error reseteando isCalculating:", err);
		}
	}
};
