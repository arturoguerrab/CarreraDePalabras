							...
							{sortedScores.map(([player, score], index) => (
								<li key={player} className="flex justify-between items-center">
									<span className="flex items-center gap-4 text-[10px] text-black uppercase truncate pr-2">
										<span
											className={`${index === 0 ? "text-yellow-500 scale-125" : "text-gray-400"} font-bold`}
										>
											#{index + 1}
										</span>
										{player}
									</span>
									<span className="text-[#ef4444] text-[10px] whitespace-nowrap">
										{score} PTS
									</span>
								</li>
							))}
							...
							<div className="flex flex-col items-center gap-6 w-full max-w-md">
								<div className="flex flex-wrap justify-center gap-3">
									{players?.map((p) => (
										<div
											key={p.email}
											className={`w-3 h-3 rounded-full border-2 border-black ${p.ready ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-gray-200"}`}
											title={p.displayName}
										/>
									))}
								</div>
								...
								<p className="text-[8px] text-gray-400 uppercase tracking-tighter text-center">
									Esperando jugadores: {players?.filter((p) => !p.ready).length}{" "}
									/ {players?.length}
								</p>
							</div>
						) : (
