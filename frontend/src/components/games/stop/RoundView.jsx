			...
			<div className="w-full max-w-4xl relative z-10">
				<div className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] rounded-3xl p-6 md:p-12 relative">
					...
					<header className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6">
						<div>
							<h1 className="text-2xl md:text-4xl text-black mb-2 leading-relaxed drop-shadow-sm uppercase">
								¡A JUGAR!
							</h1>
							<p className="text-gray-400 text-[10px] md:text-xs uppercase tracking-widest">
								Rellena los campos rápidamente
							</p>
						</div>
						...
						<div className="flex gap-4">
							...
							>
								...
								<span
									className={`text-[8px] uppercase font-bold ${timeLeft <= 10 ? "text-white" : "text-gray-500"}`}
								>
									Segundos
								</span>
							</div>
							...
							<div className="relative group">
								...
								<span className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-[8px] text-black uppercase font-bold">
									Letra
								</span>
							</div>
						</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
							{categories.map((cat) => (
								<div key={cat}>
									...
									<input
										type="text"
										defaultValue={initialValues[cat] || ""} // Cargar borrador si existe
