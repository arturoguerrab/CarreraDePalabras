  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-xl font-semibold text-indigo-700 animate-pulse">El Juez IA está deliberando...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-lg mt-10 p-6 bg-red-50 border border-red-200 rounded-xl text-center shadow-sm">
        ...
        <h3 className="font-bold text-xl text-red-800 mb-2">Error de Comunicación</h3>
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      {visual.map((bloque, idx) => (
        <section key={idx} className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 transition-all hover:shadow-2xl">
          {/* Cabecera de Categoría */}
          <div className="bg-indigo-600 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-2">
            <h2 className="text-white font-bold text-lg uppercase tracking-wider flex items-center gap-2">
              📂 {bloque.categoria}
            </h2>
            <span className="bg-indigo-800 text-indigo-100 text-xs px-3 py-1 rounded-full font-mono">
          ...
          <div className="p-6 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {bloque.respuestas.map((item, i) => (
              ...
              >
                ...
                {/* Palabra */}
                <h3 className="text-lg font-extrabold text-gray-800 mb-2 capitalize break-words">
                  {item.palabra || <span className="text-gray-400 italic text-sm">(Vacío)</span>}
                </h3>
                
                {/* Mensaje de la IA */}
                <div className={`mt-2 pt-2 border-t ${item.es_valida ? 'border-green-200' : 'border-red-200'}`}>
